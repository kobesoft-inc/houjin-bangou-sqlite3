...
1. 前回のリリースからDBをダウンロードする（分割されていれば連結する）。無ければ新規作成。
2. `build_db.py` を実行し、DBを最新の状態に同期する（詳細は次節）。
3. 何らかの更新があった場合のみ、DBが50MBを超えていれば分割し、新しいリリースを作成する。
   更新が無かった日はリリースを作成しない。
...
- DB内の `meta` テーブルに、直近取り込んだ全件データの作成日（`baseline_date`）と
  最後に適用した差分データの日付（`last_diff_date`）を保持しています。
- 公表サイト上の全件データの作成日が `baseline_date` と異なる場合（＝新しい月次データが
  公開された場合）は、全件データを再ダウンロードしてDBをフルリビルドします。
- それ以外の場合は、`last_diff_date` より後の差分データだけをダウンロードして、
  日付順に適用します（法人番号ごとにUPSERT、廃止・削除は該当行を削除）。

この方式により、毎日253MB超の全件データを再ダウンロードすることなく、数百KB程度の
差分ファイルだけで最新状態に追いつかせています。
